...
# Please add the early bird score back as i submitted a version before the early bird deadline.
# This is the improved version
def student_name():
    return "Anthony Zhang"  # Replace with your own name here
...
class BinaryPerceptron:
    ...
    def classify(self, x_vector):
        ...
        result = sum(a * b for a, b in zip(self.weights, x_vector))
    ...
    def train_with_one_example(self, x_vector, y):
        ...
        if self.classify(x_vector) == y:
            ...
            list = [a * b * c for a, b, c in zip(x_vector, [self.alpha] * len(x_vector), [y] * len(x_vector))]
            for i in range(len(list)):
                self.weights[i] += list[i]
    ...
    def train_for_an_epoch(self, training_data):
        ...
        for i in training_data:
            if self.train_with_one_example([i[0], i[1]], i[2]):
                changed_count += 1
